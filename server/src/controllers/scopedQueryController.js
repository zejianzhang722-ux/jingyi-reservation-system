...
const pendingReservations = async function(req, res) {
  try {
    ...
    if (!statuses.length) return response.error(res, '当前角色无权查看审核队列', 403);
    ...
    const listParams = params.concat([page.pageSize, page.offset]);
    const [rows] = await db.query(
      'SELECT r.*, rm.name AS room_name, rm.name AS roomName, rm.building_id, ' +
      ...
      where + ' ORDER BY r.created_at ASC, r.id ASC LIMIT ? OFFSET ?',
      listParams
    );
...
const pendingReservationCount = async function(req, res) {
  try {
    ...
    if (!statuses.length) return response.error(res, '当前角色无权查看审核队列', 403);
    const placeholders = statuses.map(function() { return '?'; }).join(',');
    let sql = 'SELECT COUNT(*) AS count FROM reservations r JOIN rooms rm ON rm.id = r.room_id WHERE r.status IN (' + placeholders + ')';
    const params = statuses.slice();
    if (!req.adminScope.isGlobal) {
      sql += ' AND rm.building_id = ?';
      params.push(req.adminScope.buildingId);
    }
