...
const updateProfileRules = [
  body('nickname').optional().isLength({ min: 1, max: 50 }).withMessage('昵称长度1-50'),
  body('phone').optional().isMobilePhone('zh-CN').withMessage('手机号格式无效'),
  body('studentId').optional().isLength({ min: 6, max: 20 }).withMessage('学号长度6-20'),
  validate
];

const roomIdRules = [
  param('id').isInt({ min: 1 }).withMessage('功能房ID无效'),
  validate
];

const reservationIdRules = [
  param('id').isInt({ min: 1 }).withMessage('预约ID无效'),
  validate
];
...
const posterRules = [
  body('title').notEmpty().withMessage('标题不能为空').isLength({ max: 100 }).withMessage('标题不能超过100字'),
  body('organization').notEmpty().withMessage('组织不能为空').isLength({ max: 100 }).withMessage('组织名不能超过100字'),
...
const violationRules = [
  body('userId').isInt({ min: 1 }).withMessage('用户ID无效'),
  body('type').notEmpty().withMessage('违规类型不能为空'),
  body('description').notEmpty().withMessage('描述不能为空'),
  body('score').isInt().withMessage('扣分值无效'),
  validate
];
...
const bindStudentRules = [
  body('studentId').notEmpty().withMessage('学号不能为空').isLength({ min: 6, max: 20 }).withMessage('学号长度6-20'),
  body('realName').notEmpty().withMessage('姓名不能为空').isLength({ max: 50 }).withMessage('姓名不能超过50字'),
  validate
];

const batchAuditRules = [
  body('ids').isArray({ min: 1 }).withMessage('请选择审核项'),
  body('ids.*').isInt({ min: 1 }).withMessage('审核ID无效'),
