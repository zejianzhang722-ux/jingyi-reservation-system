...
Component({
  ...
  observers: {
    'openHour, closeHour': function (openHour, closeHour) {
      this.buildHours(openHour, closeHour)
    },
    'seats, timeline, myReservations': function (seats, timeline, myReservations) {
      this.processTimelineData(seats, timeline, myReservations)
    }
  },
  ...
  pageLifetimes: {
    show: function () {
      var props = this.properties
      if (this.data.processedTimeline.length === 0 && props.seats && props.seats.length > 0) {
        this.processTimelineData(props.seats, props.timeline, props.myReservations)
      }
    }
  },
  ...
  methods: {
    ...
    buildBlocks: function (seatId, timeline, myReservations) {
      ...
      timeline.forEach(function (r) {
        ...
        occupiedSlots.push({ start: start, end: end, isMine: r.isMine || false })
      })

      myReservations.forEach(function (r) {
        if (seatId && r.seatId !== seatId) return
        var exists = occupiedSlots.some(function (o) {
          return o.start === r.startHour && o.end === r.endHour && o.isMine
        })
        if (!exists) {
          var start = Math.max(r.startHour, openHour)
          var end = Math.min(r.endHour, closeHour)
          occupiedSlots.push({ start: start, end: end, isMine: true })
        }
      })
      ...
      occupiedSlots.forEach(function (slot) {
        ...
        blocks.push({
          type: slot.isMine ? 'mine' : 'occupied',
          start: slot.start,
          end: slot.end
        })
      ...
      return blocks.map(function (block) {
        ...
        return {
          ...
          style: 'left:' + left + 'rpx;width:' + Math.max(width, 4) + 'rpx'
        };
      });
    },
    ...
    onBlockTouchStart: function (e) {
      ...
      var row = this.data.processedTimeline.find(function (r) {
        return String(r.seatId) === String(seatId)
      })
      ...
      this.setData({
        isSelecting: true,
        selectedSeat: seatId,
        selectedStart: Math.floor(hour * 2) / 2,
        selectedEnd: Math.ceil(hour * 2) / 2,
    ...
    onBlockTouchMove: function (e) {
      ...
      if (duration > this.data.maxHours) {
        end = start + this.data.maxHours
      }
    ...
    onBlockTouchEnd: function (e) {
      ...
      var startHour = this.data.selectedStart
      var endHour = this.data.selectedEnd
      ...
      var startHourInt = Math.floor(startHour)
      var startMinVal = (startHour % 1 >= 0.5) ? 30 : 0
      var endHourInt = Math.floor(endHour)
      var endMinVal = (endHour % 1 >= 0.5) ? 30 : 0
    ...
    getBlockClass: function (type) {
      var map = {
        free: 'timeline-block-free',
        occupied: 'timeline-block-occupied',
        mine: 'timeline-block-mine',
        unavailable: 'timeline-block-unavailable'
      }
      return 'timeline-block ' + (map[type] || '')
    }
  }
})
