...
Page({
  ...
  normalizeRoom: function (data) {
    ...
    if (!devices.length && data.facilities) {
      devices = typeof data.facilities === 'string'
        ? data.facilities.split(',').filter(function (f) { return f })
        : (data.facilities || [])
    }
    var name = data.name || ''
    return {
      ...
      openTimeDisplay: data.open_start_time || ((openHour || 8) < 10 ? '0' + (openHour || 8) : '' + (openHour || 8)) + ':00',
      closeTimeDisplay: data.open_end_time || ((closeHour || 23) < 10 ? '0' + (closeHour || 23) : '' + (closeHour || 23)) + ':00',
      devices: devices,
  ...
  loadRoomDetail: function (roomId) {
    ...
    request.get('/room/' + roomId).then(function (data) {
      ...
      that.applyRoom(localData.getRoomById(roomId))
    })
  },
  ...
  onCompareTap: function () {
    if (this.data.room) {
      wx.navigateTo({ url: '/pages/room-compare/room-compare?roomId=' + this.data.room.id })
    }
  }
})
