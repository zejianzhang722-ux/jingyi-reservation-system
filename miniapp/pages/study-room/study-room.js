...
Page({
  ...
  loadRoomInfo: function (roomId) {
    ...
    request.get('/room/' + roomId).then(function (data) {
      ...
      wx.setNavigationBarTitle({ title: data.name || '自习室预约' })
  ...
  loadTimeline: function () {
    ...
    request.get('/room/' + this.data.roomId + '/timeline', { date: this.data.selectedDate }).then(function (data) {
      ...
      var freeCount = seats.filter(function (s) {
        return !reservations.some(function (r) { return r.seatId === s.id })
      }).length
      that.setData({
        seats: seats,
  ...
  onConfirmTime: function () {
    ...
    wx.navigateTo({
      url: '/pages/reservation-confirm/reservation-confirm?roomId=' + this.data.roomId +
        ...
        '&seatName=' + encodeURIComponent(seatName)
    })
  },
