...
function mustContain(content, pattern, message) {
  if (!pattern.test(content)) fail(message)
}

function mustNotContain(content, pattern, message) {
  if (pattern.test(content)) fail(message)
}
...
mustContain(groupReserveJs, /成员将不能再退出该组团/, '组团正式提交确认弹窗应提示提交后成员不能退出')
