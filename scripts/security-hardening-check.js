...
async function main() {
  ...
  const creditRoutes = source('server/src/routes/credit.js')
  const accountBatchRoutes = source('server/src/routes/accountBatch.js')
  const statsRoutesSource = source('server/src/routes/stats.js')
  ...
  assert(/post\('\/managers'[^\n]+accountController\.createAccount/.test(adminRoutes), 'legacy manager creation route must use explicit scope validation')
  assert(/put\('\/managers\/:id'[^\n]+accountController\.updateAccount/.test(adminRoutes), 'legacy manager update route must preserve explicit scope validation')
  ...
  assert(/'admin'[^\n]+\s*'admin'\s*,\s*NULL\s*,\s*'global'/.test(seedSource), 'seed guide administrator must have explicit global scope')
  assert(/'counselor'[^\n]+\s*'counselor'\s*,\s*NULL\s*,\s*'global'/.test(seedSource), 'seed counselor must have explicit global scope')
  assert(/scope_type ENUM\('global', 'building'\)/.test(source('server/sql/schema.sql')), 'administrator schema must persist explicit scope type')
